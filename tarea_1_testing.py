...
# Error en caso de que cadena posea algo distinto
# a letras del abecedario => -200
...
def test_casos_error_filtrar_vocales():
    ...
    random_string = "abc{}123".format(punct)
    estado, res = tarea_1_example_solution.filtrar_vocales(
...
def test_casos_error_encontrar_extremos():
    ...
    # Esta línea estaba larga, ahora está partida
    estado, minimo, maximo = tarea_1_example_solution.encontrar_extremos(
        123
    )
    assert estado == -600
    ...
    # Esta línea también estaba larga, ahora partida
    estado, minimo, maximo = tarea_1_example_solution.encontrar_extremos(
        []
    )
    assert estado == -800
